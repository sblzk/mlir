...
\STATE \textbf{Method :}
\begin{enumerate}
\FORALL{Query $q_t, t=(1 \ldots T)$ }
\STATE{$X=\phi(D|q_t)$}
\STATE{$S=w_{t-1}^TX$}
\STATE{$L=sortDescendingByScore(D,S)$}
\STATE{$I=L[1:10]$}
\STATE{Display I and observe clicked elements C}
\STATE{Construct all labeled pairs $P=(x_a,x_b,y)$ from I and C}
\FORALL{i in $(1\ldots P)$}
\IF{$y_iw_{t-1}^T(x_{a_i}-x_{b_i})<1.0$ and $y_i \neq 0.0$}
\STATE{Update $w_t$ as: $w_t=w_{t-1}+\eta y_i(x_{a_i}-x_{b_i})-\eta \lambda w_{t-1}$}
\ENDIF
\ENDFOR
\ENDFOR
\RETURN $w$
...
\begin{enumerate}
\STATE{$S=w_{t-1}^TX$}
\STATE{$I'=sortDescendingByScore(D,S)$}
\STATE{$I''=Randomize(I')$}
\STATE{Initialize $I$ as empty}
\WHILE{$|I| \neq |X|$}
\STATE{With probability $\epsilon$ add first unpicked element from $I'$ to $I$}
\STATE{With probability $1-\epsilon$ add first unpicked element from $I''$ to $I$}
\ENDWHILE
\STATE{Return $I$}
\end{enumerate}
\end{algorithmic}
...
\STATE{$S=w_{t-1}^TX$}
\STATE{$I'=sortDescendingByScore(D,S)$}
\STATE{Construct $I''$ by starting from the middle of $I'$ and alternating back and forth till the list is exhausted }
\STATE{Initialize $I$ as empty}
\WHILE{$|I| \neq |X|$}
\STATE{With probability $\epsilon$ add first unpicked element from $I'$ to $I$}
\STATE{With probability $1-\epsilon$ add first unpicked element from $I''$ to $I$}
...
\STATE \textbf{Method :}
\begin{enumerate}
\STATE{Initialize Vector $w_e$ to be empty}
\STATE{Calculate mean vector $m_p$ = mean(row) for row in $W_p$}
\STATE{Uniformly sample random vector and normalize to create unit vector $u_t$}
\STATE{$w_e$ $\leftarrow$ $m_p$+$\sigma$$u_t$}
\RETURN $w_e$
\end{enumerate}
\end{algorithmic}
\end{spacing}
